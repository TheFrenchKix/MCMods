...
package lfmdevelopment.lfmclient.systems.hud;
...
import lfmdevelopment.lfmclient.systems.hud.elements.*;
import lfmdevelopment.lfmclient.systems.hud.elements.keyboard.KeyboardHud;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("lfm");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the lfm hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(lfmTextHud.WATERMARK, 4, 4, XAnchor.Left, YAnchor.Top);
        add(lfmTextHud.FPS, 4, 4 + h, XAnchor.Left, YAnchor.Top);
        add(lfmTextHud.TPS, 4, 4 + h * 2, XAnchor.Left, YAnchor.Top);
        add(lfmTextHud.PING, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(lfmTextHud.SPEED, 4, 4 + h * 4, XAnchor.Left, YAnchor.Top);

        // Top Right
        add(ActiveModulesHud.INFO, -4, 4, XAnchor.Right, YAnchor.Top);

        // Bottom Right
        add(lfmTextHud.POSITION, -4, -4, XAnchor.Right, YAnchor.Bottom);
        add(lfmTextHud.OPPOSITE_POSITION, -4, -4 - h, XAnchor.Right, YAnchor.Bottom);
        add(lfmTextHud.ROTATION, -4, -4 - h * 2, XAnchor.Right, YAnchor.Bottom);
    }

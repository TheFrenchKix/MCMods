...
package lfmdevelopment.lfmclient.systems.hud.elements;

import lfmdevelopment.lfmclient.systems.hud.Hud;
import lfmdevelopment.lfmclient.systems.hud.HudElementInfo;

public class lfmTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", lfmTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {lfm.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {lfm.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{lfm.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{lfm.name} #1{lfm.version}");
        BARITONE = addPreset("Baritone", "lfm: #1{baritone.process_name}");
    }

...
import lfmdevelopment.lfmclient.renderer.FixedUniformStorage;
...
import lfmdevelopment.lfmclient.settings.IntSetting;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> lfm = sgScreens.add(new BoolSetting.Builder()
        .name("lfm")
        .description("Applies blur to lfm screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        lfmClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] != null) {
                    fbos[i].close();
        ...
        lfmClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private void onRenderAfterWorld() {
        ...
        // Initial downsample
        renderToFbo(fbos[0], mc.getFramebuffer().getColorAttachmentView(), lfmRenderPipelines.BLUR_DOWN, ubos[0]);

        // Downsample
        for (int i = 0; i < iterations; i++) {
            renderToFbo(fbos[i + 1], fbos[i], lfmRenderPipelines.BLUR_DOWN, ubos[i + 1]);
        }

        // Upsample
        for (int i = iterations; i >= 1; i--) {
            renderToFbo(fbos[i - 1], fbos[i], lfmRenderPipelines.BLUR_UP, ubos[i - 1]);
        }

        // Render output
        MeshRenderer.begin()
            .attachments(mc.getFramebuffer())
            .pipeline(lfmRenderPipelines.BLUR_PASSTHROUGH)
            .fullscreen()
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return lfm.get();
    ...
    private static final FixedUniformStorage<BlurUniformData> UNIFORM_STORAGE = new FixedUniformStorage<>("lfm - Blur UBO", UNIFORM_SIZE, 6);

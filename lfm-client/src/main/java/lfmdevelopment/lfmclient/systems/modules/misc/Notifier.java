...
import lfmdevelopment.lfmclient.events.entity.EntityAddedEvent;
import lfmdevelopment.lfmclient.events.entity.EntityRemovedEvent;
...
import static lfmdevelopment.lfmclient.utils.player.ChatUtils.formatCoords;
    ...
    private void onEntityAdded(EntityAddedEvent event) {
        if (!event.entity.getUuid().equals(mc.player.getUuid()) && entities.get().contains(event.entity.getType()) && visualRange.get() && this.event.get() != Event.Despawn) {
            if (event.entity instanceof PlayerEntity) {
                if (!visualRangeIgnoreFriends.get() || !Friends.get().isFriend(((PlayerEntity) event.entity))) {

...
public class EntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        for (Direction direction : DirectionAccessor.lfm$getHorizontal()) {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) mc.world).lfm$getEntityLookup();
        ...
        if (entityLookup instanceof SimpleEntityLookup<Entity> simpleEntityLookup) {
            SectionedEntityCache<Entity> cache = ((SimpleEntityLookupAccessor) simpleEntityLookup).lfm$getCache();
            LongSortedSet trackedPositions = ((SectionedEntityCacheAccessor) cache).lfm$getTrackedPositions();
            Long2ObjectMap<EntityTrackingSection<Entity>> trackingSections = ((SectionedEntityCacheAccessor) cache).lfm$getTrackingSections();
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((EntityTrackingSectionAccessor) entityTrackingSection).<Entity>lfm$getCollection()) {

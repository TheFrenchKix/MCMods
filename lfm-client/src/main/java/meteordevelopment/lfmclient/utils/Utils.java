...
package lfmdevelopment.lfmclient.utils;
...
import lfmdevelopment.lfmclient.mixin.*;
import lfmdevelopment.lfmclient.mixininterface.IMinecraftClient;
import lfmdevelopment.lfmclient.settings.StatusEffectAmplifierMapSetting;
...
import lfmdevelopment.lfmclient.utils.player.EChestMemory;
import lfmdevelopment.lfmclient.utils.render.PeekScreen;
import lfmdevelopment.lfmclient.utils.render.RenderUtils;
import lfmdevelopment.lfmclient.utils.render.color.Color;
import lfmdevelopment.lfmclient.utils.world.BlockEntityIterator;
import lfmdevelopment.lfmclient.utils.world.ChunkIterator;
...
public class Utils {
    ...
    private static final ProjectionMatrix2 matrix = new ProjectionMatrix2("lfm-projection-matrix", -10, 100, true);
    ...
    public static void init() {
        lfmClient.EVENT_BUS.subscribe(Utils.class);
    }
    ...
    public static int getRenderDistance() {
        return Math.max(mc.options.getViewDistance().getValue(), ((ClientPlayNetworkHandlerAccessor) mc.getNetworkHandler()).lfm$getChunkLoadDistance());
    }
    ...
    public static void unscaledProjection() {
        ...
        RenderUtils.projection.set(((ProjectionMatrix2Accessor) matrix).lfm$callGetMatrix(width, height));
    ...
    public static void getItemsInContainerItem(ItemStack itemStack, ItemStack[] items) {
        ...
        if (components.contains(DataComponentTypes.CONTAINER)) {
            ...
            DefaultedList<ItemStack> stacks = container.lfm$getStacks();
    ...
    public static boolean hasItems(ItemStack itemStack) {
        ...
        if (container != null && !container.lfm$getStacks().isEmpty()) return true;
    ...
    public static String getWorldName() {
        ...
        if (mc.isInSingleplayer()) {
            ...
            File folder = ((MinecraftServerAccessor) mc.getServer()).lfm$getSession().getWorldDirectory(mc.world.getRegistryKey()).toFile();
    ...
    public static byte[] readBytes(InputStream in) {
        try {
            ...
            lfmClient.LOG.error("Error reading from stream.", e);
            return new byte[0];
        } finally {
    ...
    public static void leftClick() {
        ...
        int attackCooldown = ((MinecraftClientAccessor) mc).lfm$getAttackCooldown();
        if (attackCooldown == 10000) {
            ((MinecraftClientAccessor) mc).lfm$setAttackCooldown(0);
        }

        mc.options.attackKey.setPressed(true);
        ((MinecraftClientAccessor) mc).lfm$leftClick();
        mc.options.attackKey.setPressed(false);
    }

    public static void rightClick() {
        ((IMinecraftClient) mc).lfm$rightClick();
    }
    ...
    public static boolean isLoading() {
        ResourceReloadLogger.ReloadState state = ((ResourceReloadLoggerAccessor) ((MinecraftClientAccessor) mc).lfm$getResourceReloadLogger()).lfm$getReloadState();
        return state == null || !((ReloadStateAccessor) state).lfm$isFinished();
    }

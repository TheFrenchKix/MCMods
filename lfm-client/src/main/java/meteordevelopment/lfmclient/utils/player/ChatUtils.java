...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Text.empty()
            ...
            .append("[")
            .append(Text.literal("lfm").setStyle(Style.EMPTY.withColor(TextColor.fromRgb(lfmClient.ADDON.color.getPacked()))))
            .append("] ");
    }

    public static Text getlfmPrefix() {
        return PREFIX;
    }

    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned from the supplier the default lfm prefix is used.
     */
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        ...
        mc.execute(() -> ((IChatHud) mc.inGameHud.getChatHud()).lfm$add(message, finalId));
    }
    ...
    public static MutableText formatCoords(Vec3d pos) {
        ...
        if (BaritoneUtils.IS_AVAILABLE) {
            Style style = coordsText.getStyle().withFormatting(Formatting.BOLD)
                ...
                .withClickEvent(new lfmClickEvent(

...
import lfmdevelopment.lfmclient.addons.GithubRepo;
import lfmdevelopment.lfmclient.addons.lfmAddon;
import lfmdevelopment.lfmclient.gui.GuiThemes;
import lfmdevelopment.lfmclient.gui.screens.CommitsScreen;
...
import lfmdevelopment.lfmclient.utils.render.lfmToast;
...
public class TitleScreenCredits {
    ...
    private static void init() {
        // Add addons
        for (lfmAddon addon : AddonManager.ADDONS) add(addon);

        // Sort by width (lfm always first)
        credits.sort(Comparator.comparingInt(value -> value.addon == lfmClient.ADDON ? Integer.MIN_VALUE : -mc.textRenderer.getWidth(value.text)));
        ...
        lfmExecutor.execute(() -> {
            for (Credit credit : credits) {
                ...
                request.exceptionHandler(e -> lfmClient.LOG.error("Could not fetch repository information for addon '{}'.", credit.addon.name, e));
                ...
                switch (res.statusCode()) {
                    case Http.UNAUTHORIZED -> {
                        ...
                        lfmToast toast = new lfmToast.Builder("GitHub: Unauthorized").icon(Items.BARRIER).text(message).build();
                        mc.getToastManager().add(toast);
                        lfmClient.LOG.warn(message);
                        if (System.getenv("lfm.github.authorization") == null) {
                            lfmClient.LOG.info("Consider setting an authorization " +
                                "token with the 'lfm.github.authorization' environment variable.");
                            lfmClient.LOG.info("See: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens");
                        }
                    }
                    case Http.FORBIDDEN -> lfmClient.LOG.warn("Could not fetch updates for addon '{}': Rate-limited by GitHub.", credit.addon.name);
                    case Http.NOT_FOUND -> lfmClient.LOG.warn("Could not fetch updates for addon '{}': GitHub repository '{}' not found.", credit.addon.name, repo.getOwnerName());
                    case Http.SUCCESS -> {
                        if (!credit.addon.getCommit().equals(res.body().commit.sha)) {
                            synchronized (credit.text) {
                                ...
                                ((IText) ((Text) credit.text)).lfm$invalidateCache(); // ???
                            }
                        }
                    }
                }
    ...
    private static void add(lfmAddon addon) {
    ...
    private static class Credit {
        public final lfmAddon addon;
        public final MutableText text = Text.empty();

        public Credit(lfmAddon addon) {
            this.addon = addon;
        }
    }

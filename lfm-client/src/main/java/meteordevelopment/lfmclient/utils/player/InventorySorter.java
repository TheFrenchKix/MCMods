...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getScreenHandler().slots) {
            if (getInvPart(slot) == originInvPart) slots.add(new MySlot(((ISlot) slot).lfm$getId(), slot.getStack()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).lfm$getIndex();

        if (slot.inventory instanceof PlayerInventory && (!(screen instanceof CreativeInventoryScreen) || ((ISlot) slot).lfm$getId() > 8)) {

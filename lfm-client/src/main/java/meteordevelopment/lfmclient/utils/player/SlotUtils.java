...
import lfmdevelopment.lfmclient.mixin.CreativeInventoryScreenAccessor;
import lfmdevelopment.lfmclient.mixin.ItemGroupsAccessor;
import lfmdevelopment.lfmclient.mixin.MountScreenHandlerAccessor;
...
public class SlotUtils {
    ...
    private static int creativeInventory(int i) {
        if (CreativeInventoryScreenAccessor.lfm$getSelectedTab() != Registries.ITEM_GROUP.get(ItemGroupsAccessor.lfm$getInventory()))
            return -1;
    ...
    private static int horse(ScreenHandler handler, int i) {
        LivingEntity entity = ((MountScreenHandlerAccessor) handler).lfm$getMount();

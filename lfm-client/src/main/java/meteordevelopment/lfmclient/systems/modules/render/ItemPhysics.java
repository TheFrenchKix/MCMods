...
import lfmdevelopment.lfmclient.events.render.ApplyTransformationEvent;
import lfmdevelopment.lfmclient.events.render.RenderItemEntityEvent;
import lfmdevelopment.lfmclient.mixin.ItemRenderStateAccessor;
import lfmdevelopment.lfmclient.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemRenderStateAccessor) event.renderState.itemRenderState).lfm$getLayerCount(); i++) {
            ItemRenderState.LayerRenderState layer = ((ItemRenderStateAccessor) event.renderState.itemRenderState).lfm$getLayers()[i];
            ModelInfo info = getInfo(layer.getQuads());

            matrices.push();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).lfm$getTransform());

...
import lfmdevelopment.lfmclient.gui.utils.StarscriptTextBoxRenderer;
...
public class DiscordPresence extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("lfm on Crack!", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public DiscordPresence() {
        super(Categories.Misc, "discord-presence", "Displays lfm as your presence on Discord.");
    ...
    public void onActivate() {
        ...
        String largeText = "%s %s".formatted(lfmClient.NAME, lfmClient.VERSION);
        if (!lfmClient.BUILD_NUMBER.isEmpty()) largeText += " Build: " + lfmClient.BUILD_NUMBER;
        rpc.setLargeImage("lfm_client", largeText);
    ...
    private void recompile(List<String> messages, List<Script> scripts) {
        ...
        for (String message : messages) {
            Script script = lfmStarscript.compile(message);
        ...
        if (Utils.canUpdate()) {
            ...
            if (line1Ticks >= line1UpdateDelay.get() || forceUpdate) {
                if (!line1Scripts.isEmpty()) {
                    ...
                    String message = lfmStarscript.run(line1Scripts.get(i));
            ...
            if (line2Ticks >= line2UpdateDelay.get() || forceUpdate) {
                if (!line2Scripts.isEmpty()) {
                    ...
                    String message = lfmStarscript.run(line2Scripts.get(i));
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails(lfmClient.NAME + " " + (lfmClient.BUILD_NUMBER.isEmpty() ? lfmClient.VERSION : lfmClient.VERSION + " " + lfmClient.BUILD_NUMBER));
                ...
                else if (mc.currentScreen instanceof WidgetScreen) rpc.setState("Browsing lfm's GUI");
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        help.action = () -> Util.getOperatingSystem().open("https://github.com/lfmDevelopment/lfm-client/wiki/Starscript");

        return help;
    }

    private enum SmallImage {
        MineGame("thefrenchkix", "TheFrenchKix"),

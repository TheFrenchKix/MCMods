...
import lfmdevelopment.lfmclient.events.entity.player.InteractBlockEvent;
import lfmdevelopment.lfmclient.events.entity.player.InteractEntityEvent;
...
import lfmdevelopment.lfmclient.mixin.HandledScreenAccessor;
...
import lfmdevelopment.lfmclient.utils.player.*;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((HandledScreenAccessor) screen).lfm$getFocusedSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    lfmClient.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(ScreenHandler handler) {
        lfmExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        lfmExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }

...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(lfmClient.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(lfmClient.MOD_ID, file.getName());
            ...
            lfmClient.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    lfmClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    lfmClient.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            lfmClient.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);

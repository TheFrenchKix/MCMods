...
public class AutoWalkModule {
    ...
    public void tick() {
        boolean active = ModConfig.getInstance().isAutoWalkEnabled();

        if (wasActive && !active) {
            movementController.stop();
            currentPath = Collections.emptyList();
            targetPos = null;
            DebugLogger.log("AutoWalk", "Disabled");
        }
        wasActive = active;
        ...
        switch (walkState) {
            case ARRIVED -> {
                ...
                ModConfig.getInstance().setAutoWalkEnabled(false);
                currentPath = Collections.emptyList();
                targetPos = null;
            }
            case STUCK -> {
    ...
    public void render(WorldRenderContext context) {
        if (!ModConfig.getInstance().isAutoWalkEnabled()) return;

...
public class PathDebugRenderer {
    ...
    private void drawStepDestinations(MatrixStack matrices, VertexConsumer vc,
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            if (i < currentStep) {
                WorldRenderer.drawBox(matrices, vc, box, 0.15f, 0.9f, 0.15f, 0.55f); // green - done
            } else if (i == currentStep) {
                WorldRenderer.drawBox(matrices, vc, box, 1.0f, 0.55f, 0.0f, 1.0f);   // orange - active
            } else {
                WorldRenderer.drawBox(matrices, vc, box, 0.85f, 0.85f, 0.85f, 0.4f); // white - future
            }
        }
    }
    ...
    private void drawPathNodes(MatrixStack matrices, VertexConsumer vc,
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            if (i < currentIdx) {
                WorldRenderer.drawBox(matrices, vc, box, 0.0f, 0.75f, 0.9f, 0.25f);  // dim cyan - passed
            } else if (i == currentIdx) {
                WorldRenderer.drawBox(matrices, vc, box, 1.0f, 1.0f, 0.0f, 1.0f);    // bright yellow - next
            } else {
                WorldRenderer.drawBox(matrices, vc, box, 0.25f, 0.45f, 1.0f, 0.55f); // blue - upcoming
            }
        }
    }
